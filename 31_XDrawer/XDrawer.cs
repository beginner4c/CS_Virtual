    ...
    public partial class XDrawer : Form
    {
        ...
        private void Canvas_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (buttonPressed == true)
            {
                ...
                Graphics g = canvas.CreateGraphics(); // canvas 객체 안에서 사용하는 그래픽스 객체 생성
                IntPtr hdc = g.GetHdc();
                int oldMode = SetROP2(hdc, R2_NOTXORPEN); // SetROP2는 C의 Win32에서 가져온다

                _selectedFigure.draw(hdc); // 기존에 그려진 사각형을 지울 사각형을 그린다
                _selectedFigure.setXY2(newX, newY); // x2, y2, 값을 새로 세팅
                _selectedFigure.draw(hdc); // 마우스가 움직이면서 받은 새 좌표로 사각형을 그린다
